...
static void mca_btl_fftcp_frag_eager_constructor(mca_btl_fftcp_frag_t *frag)
{
    frag->size = mca_btl_fftcp_module.super.btl_eager_limit;
    frag->my_list = &mca_btl_fftcp_component.tcp_frag_eager;
}

static void mca_btl_fftcp_frag_max_constructor(mca_btl_fftcp_frag_t *frag)
{
    frag->size = mca_btl_fftcp_module.super.btl_max_send_size;
    frag->my_list = &mca_btl_fftcp_component.tcp_frag_max;
}

static void mca_btl_fftcp_frag_user_constructor(mca_btl_fftcp_frag_t *frag)
{
    frag->size = 0;
    frag->my_list = &mca_btl_fftcp_component.tcp_frag_user;
}

OBJ_CLASS_INSTANCE(mca_btl_fftcp_frag_t, mca_btl_base_descriptor_t, NULL, NULL);

OBJ_CLASS_INSTANCE(mca_btl_fftcp_frag_eager_t, mca_btl_base_descriptor_t,
                   mca_btl_fftcp_frag_eager_constructor, NULL);

OBJ_CLASS_INSTANCE(mca_btl_fftcp_frag_max_t, mca_btl_base_descriptor_t,
                   mca_btl_fftcp_frag_max_constructor, NULL);

OBJ_CLASS_INSTANCE(mca_btl_fftcp_frag_user_t, mca_btl_base_descriptor_t,
                   mca_btl_fftcp_frag_user_constructor, NULL);

size_t mca_btl_fftcp_frag_dump(mca_btl_fftcp_frag_t *frag, char *msg, char *buf, size_t length)
{
    int i, used;
...
bool mca_btl_fftcp_frag_send(mca_btl_fftcp_frag_t *frag, int sd)
{
    ssize_t cnt;
    size_t i, num_vecs;
    struct msghdr msg = {
    ...
    do {
        ...
        if (cnt < 0) {
            ...
            case EWOULDBLOCK:
                return false;
            case EFAULT:
                BTL_ERROR(("mca_btl_fftcp_frag_send: sendmsg error (%p, %lu)\n\t%s(%lu)\n",
                ...
                mca_btl_fftcp_endpoint_close(frag->endpoint);
                return false;
            default:
                BTL_PEER_ERROR(frag->endpoint->endpoint_proc->proc_opal,
                               ("mca_btl_fftcp_frag_send: sendmsg failed: %s (%d)",
...
bool mca_btl_fftcp_frag_recv(mca_btl_fftcp_frag_t *frag, int sd)
{
...
repeat:
    ...
    frag->iov_ptr[num_vecs].iov_len = mca_btl_fftcp_component.tcp_endpoint_cache
    ...
    do {
        ...
        case EWOULDBLOCK:
            return false;
        case EFAULT:
            BTL_ERROR(("mca_btl_fftcp_frag_recv: readv error (%p, %lu)\n\t%s(%lu)\n",
                       frag->iov_ptr[0].iov_base, (unsigned long) frag->iov_ptr[0].iov_len,
                       strerror(opal_socket_errno), (unsigned long) frag->iov_cnt));
            break;
        case ECONNRESET:
        ...
        default:
            BTL_PEER_ERROR(frag->endpoint->endpoint_proc->proc_opal,
                           ("mca_btl_fftcp_frag_recv: readv failed: %s (%d)",
                            strerror(opal_socket_errno), opal_socket_errno));
            break;
        }
        OPAL_THREAD_LOCK(&btl_endpoint->endpoint_send_lock);
