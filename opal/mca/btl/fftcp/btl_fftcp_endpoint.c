...
const char mca_btl_fftcp_magic_id_string[MCA_BTL_TCP_MAGIC_STRING_LENGTH] = "OPAL-TCP-BTL";

/*
 * Initialize state of the endpoint instance.
 *
 */
static void mca_btl_fftcp_endpoint_construct(mca_btl_fftcp_endpoint_t *endpoint)
{
 ...
 *
 */
static void mca_btl_fftcp_endpoint_destruct(mca_btl_fftcp_endpoint_t *endpoint)
{
    mca_btl_fftcp_endpoint_close(endpoint);
    mca_btl_fftcp_proc_remove(endpoint->endpoint_proc, endpoint);
    OBJ_DESTRUCT(&endpoint->endpoint_frags);
    OBJ_DESTRUCT(&endpoint->endpoint_send_lock);
    OBJ_DESTRUCT(&endpoint->endpoint_recv_lock);
}

OBJ_CLASS_INSTANCE(mca_btl_fftcp_endpoint_t, opal_list_item_t, mca_btl_fftcp_endpoint_construct,
                   mca_btl_fftcp_endpoint_destruct);

static void mca_btl_fftcp_endpoint_construct(mca_btl_base_endpoint_t *btl_endpoint);
static void mca_btl_fftcp_endpoint_destruct(mca_btl_base_endpoint_t *btl_endpoint);
static int mca_btl_fftcp_endpoint_start_connect(mca_btl_base_endpoint_t *);
static void mca_btl_fftcp_endpoint_connected(mca_btl_base_endpoint_t *);
static void mca_btl_fftcp_endpoint_recv_handler(int sd, short flags, void *user);
static void mca_btl_fftcp_endpoint_send_handler(int sd, short flags, void *user);
...
/**
 * The lack of protection in the mca_btl_fftcp_endpoint_dump function is voluntary
 ...
 * to the call.
 */
void mca_btl_fftcp_endpoint_dump(int level, const char *fname, int lineno, const char *funcname,
...
#    endif /* MCA_BTL_TCP_ENDPOINT_CACHE */
    ...
    if (NULL != btl_endpoint->endpoint_recv_frag)
        used += mca_btl_fftcp_frag_dump(btl_endpoint->endpoint_recv_frag, "active recv",
                                      &outmsg[used], DEBUG_LENGTH - used);
    if (used >= DEBUG_LENGTH)
        goto out;

    if (NULL != btl_endpoint->endpoint_send_frag)
        used += mca_btl_fftcp_frag_dump(btl_endpoint->endpoint_send_frag,
                                      "active send (inaccurate iov)", &outmsg[used],
                                      DEBUG_LENGTH - used);
    if (used >= DEBUG_LENGTH)
        goto out;
    OPAL_LIST_FOREACH (item, &btl_endpoint->endpoint_frags, mca_btl_fftcp_frag_t) {
        used += mca_btl_fftcp_frag_dump(item, "pending send", &outmsg[used], DEBUG_LENGTH - used);
        if (used >= DEBUG_LENGTH)
            goto out;
    }
out:
...
static inline void mca_btl_fftcp_endpoint_event_init(mca_btl_base_endpoint_t *btl_endpoint)
{
#if MCA_BTL_TCP_ENDPOINT_CACHE
    assert(NULL == btl_endpoint->endpoint_cache);
    btl_endpoint->endpoint_cache = (char *) malloc(mca_btl_fftcp_component.tcp_endpoint_cache);
    btl_endpoint->endpoint_cache_pos = btl_endpoint->endpoint_cache;
#endif /* MCA_BTL_TCP_ENDPOINT_CACHE */

    opal_event_set(mca_btl_fftcp_event_base, &btl_endpoint->endpoint_recv_event,
                   btl_endpoint->endpoint_sd, OPAL_EV_READ | OPAL_EV_PERSIST,
                   mca_btl_fftcp_endpoint_recv_handler, btl_endpoint);
    /**
     ...
     */
    opal_event_set(mca_btl_fftcp_event_base, &btl_endpoint->endpoint_send_event,
                   btl_endpoint->endpoint_sd, OPAL_EV_WRITE | OPAL_EV_PERSIST,
                   mca_btl_fftcp_endpoint_send_handler, btl_endpoint);
}
...
int mca_btl_fftcp_endpoint_send(mca_btl_base_endpoint_t *btl_endpoint, mca_btl_fftcp_frag_t *frag)
{
    int rc = OPAL_SUCCESS;
    ...
    case MCA_BTL_TCP_CLOSED:
        ...
        if (btl_endpoint->endpoint_state == MCA_BTL_TCP_CLOSED) {
            rc = mca_btl_fftcp_endpoint_start_connect(btl_endpoint);
        }
        break;
    case MCA_BTL_TCP_FAILED:
        rc = OPAL_ERR_UNREACH;
    ...
    case MCA_BTL_TCP_CONNECTED:
        if (NULL == btl_endpoint->endpoint_send_frag) {
            if (frag->base.des_flags & MCA_BTL_DES_FLAGS_PRIORITY
                && mca_btl_fftcp_frag_send(frag, btl_endpoint->endpoint_sd)) {
 ...
 */
static int mca_btl_fftcp_endpoint_send_blocking(mca_btl_base_endpoint_t *btl_endpoint,
                                              const void *data, size_t size)
{
    int ret = mca_btl_fftcp_send_blocking(btl_endpoint->endpoint_sd, data, size);
    if (ret < 0) {
        /* send-lock not needed because never called when the socket is in the
         * event set. */
        btl_endpoint->endpoint_state = MCA_BTL_TCP_FAILED;
        mca_btl_fftcp_endpoint_close(btl_endpoint);
    }
    return ret;
}
 ...
 */
static int mca_btl_fftcp_endpoint_send_connect_ack(mca_btl_base_endpoint_t *btl_endpoint)
{
    opal_process_name_t guid = opal_proc_local_get()->proc_name;
    OPAL_PROCESS_NAME_HTON(guid);

    mca_btl_fftcp_endpoint_hs_msg_t hs_msg;
    opal_string_copy(hs_msg.magic_id, mca_btl_fftcp_magic_id_string, sizeof(hs_msg.magic_id));
    hs_msg.guid = guid;

    if (sizeof(hs_msg)
        != mca_btl_fftcp_endpoint_send_blocking(btl_endpoint, &hs_msg, sizeof(hs_msg))) {
...
static void *mca_btl_fftcp_endpoint_complete_accept(int fd, int flags, void *context)
{
    ...
    if ((btl_endpoint->endpoint_sd < 0)
        ...
        btl_endpoint->endpoint_sd_next = -1;
        if (mca_btl_fftcp_endpoint_send_connect_ack(btl_endpoint) != OPAL_SUCCESS) {
        ...
        if (mca_btl_fftcp_event_base == opal_sync_event_base) {
            /* If no progress thread then raise the awarness of the default progress engine */
            opal_progress_event_users_increment();
        }
        mca_btl_fftcp_endpoint_connected(btl_endpoint);
...
void mca_btl_fftcp_endpoint_accept(mca_btl_base_endpoint_t *btl_endpoint, struct sockaddr *addr,
                                 int sd)
{
    struct timeval now = {0, 0};
    ...
    opal_event_evtimer_set(mca_btl_fftcp_event_base, &btl_endpoint->endpoint_accept_event,
                           mca_btl_fftcp_endpoint_complete_accept, btl_endpoint);
    opal_event_add(&btl_endpoint->endpoint_accept_event, &now);
}
 ...
 * been closed.
 */
void mca_btl_fftcp_endpoint_close(mca_btl_base_endpoint_t *btl_endpoint)
{
    ...
    if (MCA_BTL_TCP_CONNECTED == btl_endpoint->endpoint_state) {
        mca_btl_fftcp_hdr_t fin_msg = {
            .base.tag = 0,
            .type = MCA_BTL_TCP_HDR_TYPE_FIN,
            .count = 0,
            .size = 0,
        };
        mca_btl_fftcp_endpoint_send_blocking(btl_endpoint, &fin_msg, sizeof(fin_msg));
    }
    ...
    if (MCA_BTL_TCP_FAILED == btl_endpoint->endpoint_state) {
        mca_btl_fftcp_frag_t *frag = btl_endpoint->endpoint_send_frag;
        if (NULL == frag) {
            frag = (mca_btl_fftcp_frag_t *) opal_list_remove_first(&btl_endpoint->endpoint_frags);
        }
        while (NULL != frag) {
...
static void mca_btl_fftcp_endpoint_connected(mca_btl_base_endpoint_t *btl_endpoint)
{
    ...
    if (opal_list_get_size(&btl_endpoint->endpoint_frags) > 0) {
        if (NULL == btl_endpoint->endpoint_send_frag) {
            btl_endpoint->endpoint_send_frag = (mca_btl_fftcp_frag_t *) opal_list_remove_first(
 ...
 *  mca_btl_fftcp_endpoint_recv_handler().  Don't change them here
 *  without also changing the handling in _recv_handler()!
 */
static int mca_btl_fftcp_endpoint_recv_connect_ack(mca_btl_base_endpoint_t *btl_endpoint)
{
    size_t retval, len = strlen(mca_btl_fftcp_magic_id_string);
    mca_btl_fftcp_proc_t *btl_proc = btl_endpoint->endpoint_proc;
    opal_process_name_t guid;

    mca_btl_fftcp_endpoint_hs_msg_t hs_msg;
    retval = mca_btl_fftcp_recv_blocking(btl_endpoint->endpoint_sd, &hs_msg, sizeof(hs_msg));
...
void mca_btl_fftcp_set_socket_options(int sd)
{
#if defined(TCP_NODELAY)
    int optval;
    optval = !mca_btl_fftcp_component.tcp_not_use_nodelay;
    if (ff_setsockopt(sd, IPPROTO_TCP, TCP_NODELAY, (char *) &optval, sizeof(optval)) < 0) {
        BTL_ERROR(("ff_setsockopt(TCP_NODELAY) failed: %s (%d)", strerror(opal_socket_errno),
                   opal_socket_errno));
    }
#endif
#if defined(SO_SNDBUF)
 ...
 */
static int mca_btl_fftcp_endpoint_start_connect(mca_btl_base_endpoint_t *btl_endpoint)
{
    int rc, flags;
    ...
    mca_btl_fftcp_set_socket_options(btl_endpoint->endpoint_sd);
    ...
    if (0 == connect(btl_endpoint->endpoint_sd, (struct sockaddr *) &endpoint_addr, addrlen)) {
        ...
        if ((rc = mca_btl_fftcp_endpoint_send_connect_ack(btl_endpoint)) == OPAL_SUCCESS) {
 ...
 */
static int mca_btl_fftcp_endpoint_complete_connect(mca_btl_base_endpoint_t *btl_endpoint)
{
    int so_error = 0;
    ...
    if (getsockopt(btl_endpoint->endpoint_sd, SOL_SOCKET, SO_ERROR, (char *) &so_error, &so_length)
        ...
        opal_show_help("help-mpi-btl-tcp.txt", "socket flag fail", true, opal_process_info.nodename,
                       getpid(), "ff_fcntl(sd, F_GETFL, 0)", strerror(opal_socket_errno),
                       opal_socket_errno);
        BTL_ERROR(("ff_getsockopt() to %s:%d failed: %s (%d)",
                   opal_net_get_hostname((struct sockaddr *) &endpoint_addr),
...
static void mca_btl_fftcp_endpoint_recv_handler(int sd, short flags, void *user)
{
    ...
    switch (btl_endpoint->endpoint_state) {
    case MCA_BTL_TCP_CONNECT_ACK: {
        int rc = mca_btl_fftcp_endpoint_recv_connect_ack(btl_endpoint);
        if (OPAL_SUCCESS == rc) {
            ...
            /* Otherwise, it probably *was* an OMPI peer process on
               ...
               happened.  */
            mca_btl_fftcp_module_t *m = btl_endpoint->endpoint_btl;
    ...
    case MCA_BTL_TCP_CONNECTED: {
        mca_btl_fftcp_frag_t *frag;

        frag = btl_endpoint->endpoint_recv_frag;
        if (NULL == frag) {
            if (mca_btl_fftcp_module.super.btl_max_send_size
                > mca_btl_fftcp_module.super.btl_eager_limit) {
    ...
    data_still_pending_on_endpoint:
        ...
        if (mca_btl_fftcp_frag_recv(frag, btl_endpoint->endpoint_sd) == false) {
...
static void mca_btl_fftcp_endpoint_send_handler(int sd, short flags, void *user)
{
    mca_btl_fftcp_endpoint_t *btl_endpoint = (mca_btl_fftcp_endpoint_t *) user;
    ...
    switch (btl_endpoint->endpoint_state) {
    case MCA_BTL_TCP_CONNECTING:
        mca_btl_fftcp_endpoint_complete_connect(btl_endpoint);
        break;
    case MCA_BTL_TCP_CONNECTED:
        ...
        while (NULL != btl_endpoint->endpoint_send_frag) {
            ...
            if (mca_btl_fftcp_frag_send(frag, btl_endpoint->endpoint_sd) == false) {
                break;
            }
