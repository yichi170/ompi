...
#include "btl_fftcp.h"
#include "btl_fftcp_addr.h"
#include "btl_fftcp_endpoint.h"
#include "btl_fftcp_frag.h"
#include "btl_fftcp_proc.h"
#include "opal/constants.h"
#include "opal/mca/btl/base/base.h"
#include "opal/mca/btl/base/btl_base_error.h"
#include "opal/mca/btl/btl.h"
 ...
 * Local functions
 */
static int mca_btl_fftcp_component_register(void);
static int mca_btl_fftcp_component_open(void);
static int mca_btl_fftcp_component_close(void);

opal_event_base_t *mca_btl_fftcp_event_base = NULL;
int mca_btl_fftcp_progress_thread_trigger = -1;
int mca_btl_fftcp_pipe_to_progress[2] = {-1, -1};
static opal_thread_t mca_btl_fftcp_progress_thread = {{0}};
opal_list_t mca_btl_fftcp_ready_frag_pending_queue = {{0}};
opal_mutex_t mca_btl_fftcp_ready_frag_mutex = OPAL_MUTEX_STATIC_INIT;

mca_btl_fftcp_component_t mca_btl_fftcp_component = {
    .super = {
        ...
        .btl_version =
            {
                MCA_BTL_DEFAULT_VERSION("fftcp"),
                .mca_open_component = mca_btl_fftcp_component_open,
                .mca_close_component = mca_btl_fftcp_component_close,
                .mca_register_component_params = mca_btl_fftcp_component_register,
            },
        .btl_data =
            {/* The component is checkpoint ready */
             .param_field = MCA_BASE_METADATA_PARAM_CHECKPOINT},

        .btl_init = mca_btl_fftcp_component_init,
        .btl_progress = NULL,
    }};
...
static inline char *mca_btl_fftcp_param_register_string(const char *param_name,
                                                      const char *help_string,
                                                      const char *default_value, int level,
                                                      char **storage)
{
    *storage = (char *) default_value;
    (void) mca_base_component_var_register(&mca_btl_fftcp_component.super.btl_version, param_name,
                                           help_string, MCA_BASE_VAR_TYPE_STRING, NULL, 0, 0, level,
                                           MCA_BASE_VAR_SCOPE_READONLY, storage);
    return *storage;
}

static inline int mca_btl_fftcp_param_register_int(const char *param_name, const char *help_string,
...
static inline unsigned int mca_btl_fftcp_param_register_uint(const char *param_name,
                                                           const char *help_string,
...
struct mca_btl_fftcp_event_t {
    opal_list_item_t item;
    opal_event_t event;
};
typedef struct mca_btl_fftcp_event_t mca_btl_fftcp_event_t;

static void mca_btl_fftcp_event_construct(mca_btl_fftcp_event_t *event)
{
    MCA_BTL_TCP_CRITICAL_SECTION_ENTER(&mca_btl_fftcp_component.tcp_lock);
    opal_list_append(&mca_btl_fftcp_component.tcp_events, &event->item);
    MCA_BTL_TCP_CRITICAL_SECTION_LEAVE(&mca_btl_fftcp_component.tcp_lock);
}

static void mca_btl_fftcp_event_destruct(mca_btl_fftcp_event_t *event)
{
    MCA_BTL_TCP_CRITICAL_SECTION_ENTER(&mca_btl_fftcp_component.tcp_lock);
    opal_list_remove_item(&mca_btl_fftcp_component.tcp_events, &event->item);
    MCA_BTL_TCP_CRITICAL_SECTION_LEAVE(&mca_btl_fftcp_component.tcp_lock);
}

OBJ_CLASS_INSTANCE(mca_btl_fftcp_event_t, opal_list_item_t, mca_btl_fftcp_event_construct,
                   mca_btl_fftcp_event_destruct);

/*
 * functions for receiving event callbacks
 */
static void mca_btl_fftcp_component_recv_handler(int, short, void *);
static void mca_btl_fftcp_component_accept_handler(int, short, void *);

static int mca_btl_fftcp_component_verify(void)
{
    if (mca_btl_fftcp_component.tcp_port_min > USHRT_MAX) {
        opal_show_help("help-mpi-btl-tcp.txt", "invalid minimum port", true, "v4",
                       opal_process_info.nodename, mca_btl_fftcp_component.tcp_port_min);
        mca_btl_fftcp_component.tcp_port_min = 1024;
    }
#if OPAL_ENABLE_IPV6
    if (mca_btl_fftcp_component.tcp6_port_min > USHRT_MAX) {
        opal_show_help("help-mpi-btl-tcp.txt", "invalid minimum port", true, "v6",
                       opal_process_info.nodename, mca_btl_fftcp_component.tcp6_port_min);
        mca_btl_fftcp_component.tcp6_port_min = 1024;
    }
#endif
...
static int mca_btl_fftcp_component_register(void)
{
    ...
    mca_btl_fftcp_param_register_string(
        ...
        "btl_fftcp_if_exclude.",
        "", OPAL_INFO_LVL_1, &mca_btl_fftcp_component.tcp_if_include);
    mca_btl_fftcp_param_register_string(
        "if_exclude",
        ...
        "btl_fftcp_if_include.",
        "127.0.0.1/8,sppp", OPAL_INFO_LVL_1, &mca_btl_fftcp_component.tcp_if_exclude);

    mca_btl_fftcp_param_register_int("free_list_num", NULL, 8, OPAL_INFO_LVL_5,
                                   &mca_btl_fftcp_component.tcp_free_list_num);
    mca_btl_fftcp_param_register_int("free_list_max", NULL, -1, OPAL_INFO_LVL_5,
                                   &mca_btl_fftcp_component.tcp_free_list_max);
    mca_btl_fftcp_param_register_int("free_list_inc", NULL, 32, OPAL_INFO_LVL_5,
                                   &mca_btl_fftcp_component.tcp_free_list_inc);
    mca_btl_fftcp_param_register_int(
        "sndbuf",
        ...
        "size.",
        0, OPAL_INFO_LVL_4, &mca_btl_fftcp_component.tcp_sndbuf);
    mca_btl_fftcp_param_register_int(
        "rcvbuf",
        ...
        "size.",
        0, OPAL_INFO_LVL_4, &mca_btl_fftcp_component.tcp_rcvbuf);
    mca_btl_fftcp_param_register_int(
        "endpoint_cache",
        ...
        " endpoint_cache",
        30 * 1024, OPAL_INFO_LVL_4, &mca_btl_fftcp_component.tcp_endpoint_cache);
    mca_btl_fftcp_param_register_int("use_nagle",
                                   "Whether to use Nagle's algorithm or not (using Nagle's "
                                   "algorithm may increase short message latency)",
                                   0, OPAL_INFO_LVL_4, &mca_btl_fftcp_component.tcp_not_use_nodelay);
    mca_btl_fftcp_param_register_int(
        "port_min_v4", "The minimum port where the TCP BTL will try to bind (default 1024)", 1024,
        OPAL_INFO_LVL_2, &mca_btl_fftcp_component.tcp_port_min);
    ...
    opal_asprintf(&message,
                  ...
                  (0x1 << 16) - mca_btl_fftcp_component.tcp_port_min - 1);
    mca_btl_fftcp_param_register_int("port_range_v4", message,
                                   (0x1 << 16) - mca_btl_fftcp_component.tcp_port_min - 1,
                                   OPAL_INFO_LVL_2, &mca_btl_fftcp_component.tcp_port_range);
    free(message);
#if OPAL_ENABLE_IPV6
    mca_btl_fftcp_param_register_int(
        "port_min_v6", "The minimum port where the TCP BTL will try to bind (default 1024)", 1024,
        OPAL_INFO_LVL_2, &mca_btl_fftcp_component.tcp6_port_min);
    opal_asprintf(&message,
                  "The number of ports where the TCP BTL will try to bind (default %d)."
                  " This parameter together with the port min, define a range of ports"
                  " where Open MPI will open sockets.",
                  (0x1 << 16) - mca_btl_fftcp_component.tcp6_port_min - 1);
    mca_btl_fftcp_param_register_int("port_range_v6", message,
                                   (0x1 << 16) - mca_btl_fftcp_component.tcp6_port_min - 1,
                                   OPAL_INFO_LVL_2, &mca_btl_fftcp_component.tcp6_port_range);
    free(message);
#endif

    /* Check if we should support async progress */
    mca_btl_fftcp_param_register_int("progress_thread", NULL, 0, OPAL_INFO_LVL_1,
                                   &mca_btl_fftcp_component.tcp_enable_progress_thread);
    mca_btl_fftcp_component.report_all_unfound_interfaces = false;
    (void) mca_base_component_var_register(
        &mca_btl_fftcp_component.super.btl_version, "warn_all_unfound_interfaces",
        ...
        &mca_btl_fftcp_component.report_all_unfound_interfaces);

    mca_btl_fftcp_module.super.btl_exclusivity = MCA_BTL_EXCLUSIVITY_LOW + 100;
    mca_btl_fftcp_module.super.btl_eager_limit = 64 * 1024;
    mca_btl_fftcp_module.super.btl_rndv_eager_limit = 64 * 1024;
    mca_btl_fftcp_module.super.btl_max_send_size = 128 * 1024;
    mca_btl_fftcp_module.super.btl_rdma_pipeline_send_length = 128 * 1024;
     ...
     */
    mca_btl_fftcp_module.super.btl_rdma_pipeline_frag_size = ((1UL << 31) - 1024);
    mca_btl_fftcp_module.super.btl_min_rdma_pipeline_size = 0;
    mca_btl_fftcp_module.super.btl_flags = MCA_BTL_FLAGS_PUT | MCA_BTL_FLAGS_SEND_INPLACE
     ...
     * mca_btl_fftcp_create().
     */
    mca_btl_fftcp_module.super.btl_bandwidth = 0;
    mca_btl_fftcp_module.super.btl_latency = 0;

    mca_btl_base_param_register(&mca_btl_fftcp_component.super.btl_version,
                                &mca_btl_fftcp_module.super);
    if (mca_btl_fftcp_module.super.btl_rdma_pipeline_frag_size > ((1UL << 31) - 1024)) {
    ...
    mca_btl_fftcp_param_register_int("disable_family", NULL, 0, OPAL_INFO_LVL_2,
                                   &mca_btl_fftcp_component.tcp_disable_family);

    return mca_btl_fftcp_component_verify();
}

static int mca_btl_fftcp_component_open(void)
{
    if (OPAL_SUCCESS != mca_btl_fftcp_component_verify()) {
        return OPAL_ERROR;
    }

    /* initialize state */
    mca_btl_fftcp_component.tcp_listen_sd = -1;
#if OPAL_ENABLE_IPV6
    mca_btl_fftcp_component.tcp6_listen_sd = -1;
#endif
    mca_btl_fftcp_component.tcp_num_btls = 0;
    mca_btl_fftcp_component.tcp_addr_count = 0;
    mca_btl_fftcp_component.tcp_btls = NULL;

    /* initialize objects */
    OBJ_CONSTRUCT(&mca_btl_fftcp_component.local_ifs, opal_list_t);
    OBJ_CONSTRUCT(&mca_btl_fftcp_component.tcp_lock, opal_mutex_t);
    OBJ_CONSTRUCT(&mca_btl_fftcp_component.tcp_procs, opal_proc_table_t);
    OBJ_CONSTRUCT(&mca_btl_fftcp_component.tcp_events, opal_list_t);
    OBJ_CONSTRUCT(&mca_btl_fftcp_component.tcp_frag_eager, opal_free_list_t);
    OBJ_CONSTRUCT(&mca_btl_fftcp_component.tcp_frag_max, opal_free_list_t);
    OBJ_CONSTRUCT(&mca_btl_fftcp_component.tcp_frag_user, opal_free_list_t);
    opal_proc_table_init(&mca_btl_fftcp_component.tcp_procs, 16, 256);

    OBJ_CONSTRUCT(&mca_btl_fftcp_component.tcp_frag_eager_mutex, opal_mutex_t);
    OBJ_CONSTRUCT(&mca_btl_fftcp_component.tcp_frag_max_mutex, opal_mutex_t);
    OBJ_CONSTRUCT(&mca_btl_fftcp_component.tcp_frag_user_mutex, opal_mutex_t);
    OBJ_CONSTRUCT(&mca_btl_fftcp_ready_frag_mutex, opal_mutex_t);
    OBJ_CONSTRUCT(&mca_btl_fftcp_ready_frag_pending_queue, opal_list_t);

    /* if_include and if_exclude need to be mutually exclusive */
    if (OPAL_SUCCESS
        != mca_base_var_check_exclusive("opal",
                                        mca_btl_fftcp_component.super.btl_version.mca_type_name,
                                        mca_btl_fftcp_component.super.btl_version.mca_component_name,
                                        "if_include",
                                        mca_btl_fftcp_component.super.btl_version.mca_type_name,
                                        mca_btl_fftcp_component.super.btl_version.mca_component_name,
                                        "if_exclude")) {
...
static int mca_btl_fftcp_component_close(void)
{
    mca_btl_fftcp_event_t *event, *next;

    /**
     * If we have a progress thread we should shut it down before
     * moving forward with the TCP tearing down process.
     */
    if ((NULL != mca_btl_fftcp_event_base) && (mca_btl_fftcp_event_base != opal_sync_event_base)) {
        /* Turn of the progress thread before moving forward */
        if (-1 != mca_btl_fftcp_progress_thread_trigger) {
            void *ret = NULL; /* not currently used */

            mca_btl_fftcp_progress_thread_trigger = 0;
            /* Let the progress thread know that we're going away */
            if (-1 != mca_btl_fftcp_pipe_to_progress[1]) {
                close(mca_btl_fftcp_pipe_to_progress[1]);
                mca_btl_fftcp_pipe_to_progress[1] = -1;
            }
            /* wait until the TCP progress thread completes */
            opal_thread_join(&mca_btl_fftcp_progress_thread, &ret);
            assert(-1 == mca_btl_fftcp_progress_thread_trigger);
        }
        opal_event_del(&mca_btl_fftcp_component.tcp_recv_thread_async_event);
        opal_event_base_free(mca_btl_fftcp_event_base);
        mca_btl_fftcp_event_base = NULL;

        /* Close the remaining pipes */
        if (-1 != mca_btl_fftcp_pipe_to_progress[0]) {
            close(mca_btl_fftcp_pipe_to_progress[0]);
            mca_btl_fftcp_pipe_to_progress[0] = -1;
        }
    }

    OBJ_DESTRUCT(&mca_btl_fftcp_component.tcp_frag_eager_mutex);
    OBJ_DESTRUCT(&mca_btl_fftcp_component.tcp_frag_max_mutex);

    OBJ_DESTRUCT(&mca_btl_fftcp_ready_frag_mutex);
    OBJ_DESTRUCT(&mca_btl_fftcp_ready_frag_pending_queue);

    if (NULL != mca_btl_fftcp_component.tcp_btls) {
        free(mca_btl_fftcp_component.tcp_btls);
    }

    if (mca_btl_fftcp_component.tcp_listen_sd >= 0) {
        opal_event_del(&mca_btl_fftcp_component.tcp_recv_event);
        CLOSE_THE_SOCKET(mca_btl_fftcp_component.tcp_listen_sd);
        mca_btl_fftcp_component.tcp_listen_sd = -1;
    }
#if OPAL_ENABLE_IPV6
    if (mca_btl_fftcp_component.tcp6_listen_sd >= 0) {
        opal_event_del(&mca_btl_fftcp_component.tcp6_recv_event);
        CLOSE_THE_SOCKET(mca_btl_fftcp_component.tcp6_listen_sd);
        mca_btl_fftcp_component.tcp6_listen_sd = -1;
    }
#endif
    ...
    OPAL_LIST_FOREACH_SAFE (event, next, &mca_btl_fftcp_component.tcp_events, mca_btl_fftcp_event_t) {
        opal_event_del(&event->event);
        OBJ_RELEASE(event);
    }

    opal_proc_table_remove_value(&mca_btl_fftcp_component.tcp_procs,
                                 opal_proc_local_get()->proc_name);

    /* release resources */
    OBJ_DESTRUCT(&mca_btl_fftcp_component.tcp_procs);
    OBJ_DESTRUCT(&mca_btl_fftcp_component.tcp_frag_eager);
    OBJ_DESTRUCT(&mca_btl_fftcp_component.tcp_frag_max);
    OBJ_DESTRUCT(&mca_btl_fftcp_component.tcp_frag_user);
    OBJ_DESTRUCT(&mca_btl_fftcp_component.tcp_lock);
    OPAL_LIST_DESTRUCT(&mca_btl_fftcp_component.local_ifs);
...
static int mca_btl_fftcp_create(const int if_kindex, const char *if_name)
{
    struct mca_btl_fftcp_module_t *btl;
     ...
     * device into mca_btl_fftcp_create().  This is a cleanup of the
    ...
    OPAL_LIST_FOREACH (selected_interface, &opal_if_list, opal_if_t) {
        ...
        if (addr.ss_family == AF_INET && 4 != mca_btl_fftcp_component.tcp_disable_family) {
            found = true;
            break;
        } else if (addr.ss_family == AF_INET6 && 6 != mca_btl_fftcp_component.tcp_disable_family) {
            found = true;
            break;
        }
    }
    ...
    for (i = 0; i < (int) mca_btl_fftcp_component.tcp_num_links; i++) {
        btl = (struct mca_btl_fftcp_module_t *) malloc(sizeof(mca_btl_fftcp_module_t));
        if (NULL == btl) {
        ...
        memcpy(btl, &mca_btl_fftcp_module, sizeof(mca_btl_fftcp_module));
        OBJ_CONSTRUCT(&btl->tcp_endpoints, opal_list_t);
        OBJ_CONSTRUCT(&btl->tcp_endpoints_mutex, opal_mutex_t);
        mca_btl_fftcp_component.tcp_btls[mca_btl_fftcp_component.tcp_num_btls++] = btl;

        /* initialize the btl */
        /* This index is used as a key for a hash table used for interface matching. */
        btl->btl_index = mca_btl_fftcp_component.tcp_num_btls - 1;
        ...
        mca_btl_fftcp_param_register_uint(param, NULL, btl->super.btl_bandwidth, OPAL_INFO_LVL_5,
                                        &btl->super.btl_bandwidth);

        /* allow user to override/specify latency ranking */
        sprintf(param, "latency_%s", if_name);
        mca_btl_fftcp_param_register_uint(param, NULL, btl->super.btl_latency, OPAL_INFO_LVL_5,
        ...
        opal_list_append(&mca_btl_fftcp_component.local_ifs, &(copied_interface->super));
...
static char **split_and_resolve(char **orig_str, char *name, bool reqd)
{
    ...
    for (i = 0; NULL != argv[i]; ++i) {
        ...
        *str = '\0';
        ...
        if (0 == match_count) {
            if (reqd || mca_btl_fftcp_component.report_all_unfound_interfaces) {
                opal_show_help("help-mpi-btl-tcp.txt", "invalid if_inexclude", true, name,
                               opal_process_info.nodename, tmp,
...
static int mca_btl_fftcp_component_create_instances(void)
{
    ...
    mca_btl_fftcp_component.tcp_btls = (mca_btl_fftcp_module_t **) malloc(
        mca_btl_fftcp_component.tcp_num_links * kif_count * sizeof(mca_btl_fftcp_module_t *));
    if (NULL == mca_btl_fftcp_component.tcp_btls) {
        ret = OPAL_ERR_OUT_OF_RESOURCE;
        goto cleanup;
    }

    mca_btl_fftcp_component.tcp_addr_count = if_count;

    /* if the user specified an interface list - use these exclusively */
    argv = include = split_and_resolve(&mca_btl_fftcp_component.tcp_if_include, "include", true);
    while (argv && *argv) {
        char *if_name = *argv;
     ...
     */
    exclude = split_and_resolve(&mca_btl_fftcp_component.tcp_if_exclude, "exclude", false);
    {
        int i;
        for (i = 0; i < kif_count; i++) {
            ...
            if (argv == 0 || *argv == 0) {
                mca_btl_fftcp_create(if_index, if_name);
            }
        }
    }
...
static void *mca_btl_fftcp_progress_thread_engine(opal_object_t *obj)
{
    opal_thread_t *current_thread = (opal_thread_t *) obj;

    while (1 == (*((int *) current_thread->t_arg))) {
        opal_event_loop(mca_btl_fftcp_event_base, OPAL_EVLOOP_ONCE);
    }
    (*((int *) current_thread->t_arg)) = -1;
    return NULL;
}

static void mca_btl_fftcp_component_event_async_handler(int fd, short unused, void *context)
{
    opal_event_t *event;
    int rc;

    rc = read(fd, (void *) &event, sizeof(opal_event_t *));
    assert(fd == mca_btl_fftcp_pipe_to_progress[0]);
    if (0 == rc) {
...
static int mca_btl_fftcp_component_create_listen(uint16_t af_family)
{
    int flags, sd, rc;
...
#if OPAL_ENABLE_IPV6
    {
        ...
        if ((rc = getaddrinfo(NULL, "0", &hints, &res))) {
            opal_output(0, "mca_btl_fftcp_create_listen: unable to resolve. %s\n", gai_strerror(rc));
            CLOSE_THE_SOCKET(sd);
            return OPAL_ERROR;
        }
...
#    ifdef IPV6_V6ONLY
        ...
        if (AF_INET6 == af_family) {
            ...
            if (setsockopt(sd, IPPROTO_IPV6, IPV6_V6ONLY, (char *) &flg, sizeof(flg)) < 0) {
                BTL_ERROR(("mca_btl_fftcp_create_listen: unable to set IPV6_V6ONLY\n"));
            }
        }
#    endif /* IPV6_V6ONLY */
    }
...

    {
...
#if OPAL_ENABLE_IPV6
        if (AF_INET6 == af_family) {
            range = mca_btl_fftcp_component.tcp6_port_range;
            port = mca_btl_fftcp_component.tcp6_port_min;
        } else
#endif /* OPAL_ENABLE_IPV6 */
        {
            range = mca_btl_fftcp_component.tcp_port_range;
            port = mca_btl_fftcp_component.tcp_port_min;
        }
        ...
        if (AF_INET6 == af_family) {
            BTL_ERROR(("bind6() failed: no port available in the range [%d..%d]",
                       mca_btl_fftcp_component.tcp6_port_min,
                       mca_btl_fftcp_component.tcp6_port_min + range));
        } else
#endif /* OPAL_ENABLE_IPV6 */
        {
            BTL_ERROR(("bind() failed: no port available in the range [%d..%d]",
                       mca_btl_fftcp_component.tcp_port_min,
                       mca_btl_fftcp_component.tcp_port_min + range));
        }
        CLOSE_THE_SOCKET(sd);
        return OPAL_ERROR;
    }
socket_binded:
...
#if OPAL_ENABLE_IPV6
    if (AF_INET6 == af_family) {
        mca_btl_fftcp_component.tcp6_listen_port = ((struct sockaddr_in6 *) &inaddr)->sin6_port;
        mca_btl_fftcp_component.tcp6_listen_sd = sd;
        opal_output_verbose(30, opal_btl_base_framework.framework_output,
                            "btl:tcp: my listening v6 socket port is %d",
                            ntohs(mca_btl_fftcp_component.tcp6_listen_port));
    } else
#endif
    {
        char str[16];
        mca_btl_fftcp_component.tcp_listen_port = ((struct sockaddr_in *) &inaddr)->sin_port;
        mca_btl_fftcp_component.tcp_listen_sd = sd;
        inet_ntop(AF_INET, &(((struct sockaddr_in *) &inaddr)->sin_addr), str, sizeof(str));
        opal_output_verbose(30, opal_btl_base_framework.framework_output,
                            "btl:tcp: my listening v4 socket is %s:%u", str,
                            ntohs(mca_btl_fftcp_component.tcp_listen_port));
    }
    ...
    if (mca_btl_fftcp_component.tcp_enable_progress_thread) {
        /* Declare our intent to use threads. */
        opal_event_use_threads();
        if (NULL == mca_btl_fftcp_event_base) {
            /* fall back to only one event base (the one shared by the entire Open MPI framework) */

            if (NULL == (mca_btl_fftcp_event_base = opal_event_base_create())) {
                BTL_ERROR(("BTL TCP failed to create progress event base"));
                goto move_forward_with_no_thread;
            }
            opal_event_base_priority_init(mca_btl_fftcp_event_base, OPAL_EVENT_NUM_PRI);

            /* construct the thread object */
            OBJ_CONSTRUCT(&mca_btl_fftcp_progress_thread, opal_thread_t);

            /**
             * Create a pipe to communicate between the main thread and the progress thread.
             */
            if (0 != pipe(mca_btl_fftcp_pipe_to_progress)) {
                opal_event_base_free(mca_btl_fftcp_event_base);
                /* fall back to only one event base (the one shared by the entire Open MPI framework
                 */
                mca_btl_fftcp_event_base = opal_sync_event_base;
                mca_btl_fftcp_progress_thread_trigger = -1; /* thread not started */
                goto move_forward_with_no_thread;
            }
            /* setup the receiving end of the pipe as non-blocking */
            if ((flags = fcntl(mca_btl_fftcp_pipe_to_progress[0], F_GETFL, 0)) < 0) {
                BTL_ERROR(("fcntl(F_GETFL) failed: %s (%d)", strerror(opal_socket_errno),
                           opal_socket_errno));
            } else {
                flags |= O_NONBLOCK;
                if (fcntl(mca_btl_fftcp_pipe_to_progress[0], F_SETFL, flags) < 0)
                    BTL_ERROR(("fcntl(F_SETFL) failed: %s (%d)", strerror(opal_socket_errno),
                               opal_socket_errno));
            }
            /* Progress thread event */
            opal_event_set(mca_btl_fftcp_event_base,
                           &mca_btl_fftcp_component.tcp_recv_thread_async_event,
                           mca_btl_fftcp_pipe_to_progress[0], OPAL_EV_READ | OPAL_EV_PERSIST,
                           mca_btl_fftcp_component_event_async_handler, &mca_btl_fftcp_progress_thread);
            opal_event_add(&mca_btl_fftcp_component.tcp_recv_thread_async_event, 0);

            /* fork off a thread to progress it */
            mca_btl_fftcp_progress_thread.t_run = mca_btl_fftcp_progress_thread_engine;
            mca_btl_fftcp_progress_thread.t_arg = &mca_btl_fftcp_progress_thread_trigger;
            mca_btl_fftcp_progress_thread_trigger = 1; /* thread up and running */
            if (OPAL_SUCCESS != (rc = opal_thread_start(&mca_btl_fftcp_progress_thread))) {
    ...
    if (AF_INET == af_family) {
        opal_event_set(mca_btl_fftcp_event_base, &mca_btl_fftcp_component.tcp_recv_event,
                       mca_btl_fftcp_component.tcp_listen_sd, OPAL_EV_READ | OPAL_EV_PERSIST,
                       mca_btl_fftcp_component_accept_handler, 0);
        MCA_BTL_TCP_ACTIVATE_EVENT(&mca_btl_fftcp_component.tcp_recv_event, 0);
    }
#if OPAL_ENABLE_IPV6
    if (AF_INET6 == af_family) {
        opal_event_set(mca_btl_fftcp_event_base, &mca_btl_fftcp_component.tcp6_recv_event,
                       mca_btl_fftcp_component.tcp6_listen_sd, OPAL_EV_READ | OPAL_EV_PERSIST,
                       mca_btl_fftcp_component_accept_handler, 0);
...
static int mca_btl_fftcp_component_exchange(void)
{
    int rc;
    size_t i;
    size_t num_btls = mca_btl_fftcp_component.tcp_num_btls;
    size_t size = num_btls * sizeof(mca_btl_fftcp_modex_addr_t);
    mca_btl_fftcp_modex_addr_t *addrs;

    if (num_btls <= 0) {
        return 0;
    }

    addrs = (mca_btl_fftcp_modex_addr_t *) malloc(size);
    if (NULL == addrs) {
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
    memset(addrs, 0, size);
    ...
    for (i = 0; i < num_btls; i++) {
        struct mca_btl_fftcp_module_t *btl = mca_btl_fftcp_component.tcp_btls[i];
        ...
        if (AF_INET6 == addr->sa_family) {
            ...
            addrs[i].addr_port = mca_btl_fftcp_component.tcp6_listen_port;
            ...
            addrs[i].addr_port = mca_btl_fftcp_component.tcp_listen_port;
 ...
 */
mca_btl_base_module_t **mca_btl_fftcp_component_init(int *num_btl_modules,
                                                   ...
                                                   bool enable_mpi_threads)
{
    ...
    *num_btl_modules = 0;
    ...
    opal_free_list_init(&mca_btl_fftcp_component.tcp_frag_eager,
                        sizeof(mca_btl_fftcp_frag_eager_t) + mca_btl_fftcp_module.super.btl_eager_limit,
                        opal_cache_line_size, OBJ_CLASS(mca_btl_fftcp_frag_eager_t), 0,
                        opal_cache_line_size, mca_btl_fftcp_component.tcp_free_list_num,
                        mca_btl_fftcp_component.tcp_free_list_max,
                        mca_btl_fftcp_component.tcp_free_list_inc, NULL, 0, NULL, NULL, NULL);

    opal_free_list_init(&mca_btl_fftcp_component.tcp_frag_max,
                        sizeof(mca_btl_fftcp_frag_max_t) + mca_btl_fftcp_module.super.btl_max_send_size,
                        opal_cache_line_size, OBJ_CLASS(mca_btl_fftcp_frag_max_t), 0,
                        opal_cache_line_size, mca_btl_fftcp_component.tcp_free_list_num,
                        mca_btl_fftcp_component.tcp_free_list_max,
                        mca_btl_fftcp_component.tcp_free_list_inc, NULL, 0, NULL, NULL, NULL);

    opal_free_list_init(&mca_btl_fftcp_component.tcp_frag_user, sizeof(mca_btl_fftcp_frag_user_t),
                        opal_cache_line_size, OBJ_CLASS(mca_btl_fftcp_frag_user_t), 0,
                        opal_cache_line_size, mca_btl_fftcp_component.tcp_free_list_num,
                        mca_btl_fftcp_component.tcp_free_list_max,
                        mca_btl_fftcp_component.tcp_free_list_inc, NULL, 0, NULL, NULL, NULL);
...
#if OPAL_ENABLE_IPV6
    if ((ret = mca_btl_fftcp_component_create_listen(AF_INET6)) != OPAL_SUCCESS) {
        if (!(OPAL_ERR_IN_ERRNO == ret && EAFNOSUPPORT == opal_socket_errno)) {
            opal_output(0, "mca_btl_fftcp_component: IPv6 listening socket failed\n");
            return 0;
        }
    }
#endif
    ...
    if (OPAL_SUCCESS != (ret = mca_btl_fftcp_component_exchange())) {
        return 0;
    }
    btls = (mca_btl_base_module_t **) malloc(mca_btl_fftcp_component.tcp_num_btls
    ...
    if (0 < mca_btl_fftcp_progress_thread_trigger) {
        for (i = 0; i < mca_btl_fftcp_component.tcp_num_btls; i++) {
            mca_btl_fftcp_component.tcp_btls[i]->super.btl_flags
    ...
    if (mca_btl_fftcp_component.tcp_num_btls > 1
        && (enable_mpi_threads || 0 < mca_btl_fftcp_progress_thread_trigger)) {
        for (i = 0; i < mca_btl_fftcp_component.tcp_num_btls; i++) {
            mca_btl_fftcp_component.tcp_btls[i]->super.btl_flags |= MCA_BTL_FLAGS_SINGLE_ADD_PROCS;
        }
    }

    memcpy(btls, mca_btl_fftcp_component.tcp_btls,
           mca_btl_fftcp_component.tcp_num_btls * sizeof(mca_btl_fftcp_module_t *));
    *num_btl_modules = mca_btl_fftcp_component.tcp_num_btls;
    return btls;
}
 ...
 */
static void mca_btl_fftcp_component_accept_handler(int incoming_sd, short ignored, void *unused)
{
    while (true) {
#if OPAL_ENABLE_IPV6
        ...
        assert(NULL != mca_btl_fftcp_event_base);
        /* wait for receipt of peers process identifier to complete this connection */
        event = OBJ_NEW(mca_btl_fftcp_event_t);
        opal_event_set(mca_btl_fftcp_event_base, &(event->event), sd, OPAL_EV_READ,
                       mca_btl_fftcp_component_recv_handler, event);
 ...
 * protocol.
 */
static void mca_btl_fftcp_component_recv_handler(int sd, short flags, void *user)
{
    mca_btl_fftcp_event_t *event = (mca_btl_fftcp_event_t *) user;
    opal_process_name_t guid;
    struct sockaddr_storage addr;
    opal_socklen_t addr_len = sizeof(addr);
    mca_btl_fftcp_proc_t *btl_proc;
    bool sockopt = true;
    size_t retval, len = strlen(mca_btl_fftcp_magic_id_string);
    mca_btl_fftcp_endpoint_hs_msg_t hs_msg;
    ...
    OBJ_RELEASE(event);
    retval = mca_btl_fftcp_recv_blocking(sd, (void *) &hs_msg, sizeof(hs_msg));
    ...
    guid = hs_msg.guid;
    if (0 != strncmp(hs_msg.magic_id, mca_btl_fftcp_magic_id_string, len)) {
        const char *peer = opal_fd_get_peer_name(sd);
        opal_output_verbose(
            20, opal_btl_base_framework.framework_output,
            ...
            peer, mca_btl_fftcp_magic_id_string, hs_msg.magic_id);
        free((char *) peer);
    ...
    btl_proc = mca_btl_fftcp_proc_lookup(&guid);
    if (NULL == btl_proc) {
    ...
    (void) mca_btl_fftcp_proc_accept(btl_proc, (struct sockaddr *) &addr, sd);
