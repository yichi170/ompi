...
static int mca_btl_fftcp_register_error_cb(struct mca_btl_base_module_t *btl,
                                         mca_btl_base_module_error_cb_fn_t cbfunc);

mca_btl_fftcp_module_t mca_btl_fftcp_module =
    {.super =
         {
             .btl_component = &mca_btl_fftcp_component.super,
             .btl_add_procs = mca_btl_fftcp_add_procs,
             .btl_del_procs = mca_btl_fftcp_del_procs,
             .btl_finalize = mca_btl_fftcp_finalize,
             .btl_alloc = mca_btl_fftcp_alloc,
             .btl_free = mca_btl_fftcp_free,
             .btl_prepare_src = mca_btl_fftcp_prepare_src,
             .btl_send = mca_btl_fftcp_send,
             .btl_put = mca_btl_fftcp_put,
             .btl_dump = mca_btl_base_dump,
             .btl_register_error = mca_btl_fftcp_register_error_cb, /* register error */
         },
...
static int mca_btl_fftcp_register_error_cb(struct mca_btl_base_module_t *btl,
                                         mca_btl_base_module_error_cb_fn_t cbfunc)
{
    mca_btl_fftcp_module_t *tcp_btl = (mca_btl_fftcp_module_t *) btl;
...
int mca_btl_fftcp_add_procs(struct mca_btl_base_module_t *btl, size_t nprocs,
                          ...
                          opal_bitmap_t *reachable)
{
    ...
    for (i = 0; i < (int) nprocs; i++) {
        ...
        if (NULL == (tcp_proc = mca_btl_fftcp_proc_create(opal_proc))) {
            continue;
        }
        ...
        if (!existing_found) {
             ...
             */
            tcp_endpoint = OBJ_NEW(mca_btl_fftcp_endpoint_t);
            ...
            rc = mca_btl_fftcp_proc_insert(tcp_proc, tcp_endpoint);
            if (rc != OPAL_SUCCESS) {
                OPAL_THREAD_UNLOCK(&tcp_proc->proc_lock);
...
int mca_btl_fftcp_del_procs(struct mca_btl_base_module_t *btl, size_t nprocs,
                          struct opal_proc_t **procs, struct mca_btl_base_endpoint_t **endpoints)
{
    ...
    OPAL_THREAD_LOCK(&tcp_btl->tcp_endpoints_mutex);
    for (i = 0; i < nprocs; i++) {
        mca_btl_fftcp_endpoint_t *tcp_endpoint = endpoints[i];
...
mca_btl_base_descriptor_t *mca_btl_fftcp_alloc(struct mca_btl_base_module_t *btl,
                                             struct mca_btl_base_endpoint_t *endpoint,
                                             uint8_t order, size_t size, uint32_t flags)
{
    mca_btl_fftcp_frag_t *frag = NULL;
    ...
    frag->btl = (mca_btl_fftcp_module_t *) btl;
...
int mca_btl_fftcp_free(struct mca_btl_base_module_t *btl, mca_btl_base_descriptor_t *des)
{
    mca_btl_fftcp_frag_t *frag = (mca_btl_fftcp_frag_t *) des;
 ...
 */
mca_btl_base_descriptor_t *mca_btl_fftcp_prepare_src(struct mca_btl_base_module_t *btl,
                                                   struct mca_btl_base_endpoint_t *endpoint,
                                                   ...
                                                   uint32_t flags)
{
    ...
    if (opal_convertor_need_buffers(convertor) || opal_convertor_on_device(convertor)) {
        ...
        if (OPAL_UNLIKELY(rc < 0)) {
            mca_btl_fftcp_free(btl, &frag->base);
            return NULL;
        }
...
int mca_btl_fftcp_send(struct mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
                     struct mca_btl_base_descriptor_t *descriptor, mca_btl_base_tag_t tag)
{
    mca_btl_fftcp_module_t *tcp_btl = (mca_btl_fftcp_module_t *) btl;
    mca_btl_fftcp_frag_t *frag = (mca_btl_fftcp_frag_t *) descriptor;
    int i;
    ...
    return mca_btl_fftcp_endpoint_send(endpoint, frag);
}
...
{
    mca_btl_fftcp_frag_t *frag = (mca_btl_fftcp_frag_t *) desc;
...
int mca_btl_fftcp_put(mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
                    ...
                    void *cbdata)
{
    ...
    return ((i = mca_btl_fftcp_endpoint_send(endpoint, frag)) >= 0 ? OPAL_SUCCESS : i);
}
...
int mca_btl_fftcp_get(mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
                    void *local_address, uint64_t remote_address,
                    mca_btl_base_registration_handle_t *local_handle,
                    mca_btl_base_registration_handle_t *remote_handle, size_t size, int flags,
                    int order, mca_btl_base_rdma_completion_fn_t cbfunc, void *cbcontext,
                    void *cbdata)
{
    ...
    return ((rc = mca_btl_fftcp_endpoint_send(endpoint, frag)) >= 0 ? OPAL_SUCCESS : rc);
}
...
int mca_btl_fftcp_finalize(struct mca_btl_base_module_t *btl)
{
    mca_btl_fftcp_module_t *tcp_btl = (mca_btl_fftcp_module_t *) btl;
    ...
    for (item = opal_list_remove_first(&tcp_btl->tcp_endpoints); item != NULL;
        ...
        mca_btl_fftcp_endpoint_t *endpoint = (mca_btl_fftcp_endpoint_t *) item;
...
void mca_btl_fftcp_dump(struct mca_btl_base_module_t *base_btl,
                      struct mca_btl_base_endpoint_t *endpoint, int verbose)
{
    mca_btl_fftcp_module_t *btl = (mca_btl_fftcp_module_t *) base_btl;
...
int mca_btl_fftcp_recv_blocking(int sd, void *data, size_t size)
{
...
int mca_btl_fftcp_send_blocking(int sd, const void *data, size_t size)
{
    unsigned char *ptr = (unsigned char *) data;
    size_t cnt = 0;
    while (cnt < size) {
