...
static void mca_btl_fftcp_proc_construct(mca_btl_fftcp_proc_t *proc);
static void mca_btl_fftcp_proc_destruct(mca_btl_fftcp_proc_t *proc);

OBJ_CLASS_INSTANCE(mca_btl_fftcp_proc_t, opal_list_item_t, mca_btl_fftcp_proc_construct,
                   mca_btl_fftcp_proc_destruct);

void mca_btl_fftcp_proc_construct(mca_btl_fftcp_proc_t *tcp_proc)
{
    ...
    opal_hash_table_init(&tcp_proc->btl_index_to_endpoint, mca_btl_fftcp_component.tcp_num_btls);
}
...
void mca_btl_fftcp_proc_destruct(mca_btl_fftcp_proc_t *tcp_proc)
{
    if (NULL != tcp_proc->proc_opal) {
        /* remove from list of all proc instances */
        OPAL_THREAD_LOCK(&mca_btl_fftcp_component.tcp_lock);
        opal_proc_table_remove_value(&mca_btl_fftcp_component.tcp_procs,
                                     tcp_proc->proc_opal->proc_name);
        OPAL_THREAD_UNLOCK(&mca_btl_fftcp_component.tcp_lock);
...
static inline int mca_btl_fftcp_proc_is_proc_left(opal_process_name_t a, opal_process_name_t b)
{
    if (a.jobid != b.jobid) {
...
#define MCA_BTL_TCP_PROC_REMOTE_VERTEX(index) (index + mca_btl_fftcp_component.tcp_num_btls)
 ...
 *
 */
static int mca_btl_fftcp_proc_create_interface_graph(mca_btl_fftcp_proc_t *btl_proc,
                                                   mca_btl_fftcp_modex_addr_t *remote_addrs,
                                                   int local_proc_is_left,
                                                   opal_bp_graph_t **graph_out)
{
    opal_bp_graph_t *graph = NULL;
    opal_reachable_t *results = NULL;
    opal_list_t *local_list = &mca_btl_fftcp_component.local_ifs;
    ...
    for (x = 0; x < results->num_local; x++) {
        rc = opal_bp_graph_add_vertex(graph, &mca_btl_fftcp_component.tcp_btls[x]->btl_index,
                                      &v_index);
        if (OPAL_SUCCESS != rc) {
            goto err_graph;
        }
 ...
 * a pointer to a mca_btl_fftcp_addr_t struct.
 */
static int mca_btl_fftcp_proc_store_matched_interfaces(mca_btl_fftcp_proc_t *btl_proc,
                                                     ...
                                                     int num_matched, int *matched_edges)
{
    ...
    struct mca_btl_fftcp_addr_t *remote_addr;
...
static int mca_btl_fftcp_proc_handle_modex_addresses(mca_btl_fftcp_proc_t *btl_proc,
                                                   mca_btl_fftcp_modex_addr_t *remote_addrs,
                                                   int local_proc_is_left)
{
    opal_bp_graph_t *graph = NULL;
    ...
    rc = mca_btl_fftcp_proc_create_interface_graph(btl_proc, remote_addrs, local_proc_is_left,
                                                 &graph);
    if (rc) {
        goto cleanup;
    }
    ...
    rc = mca_btl_fftcp_proc_store_matched_interfaces(btl_proc, local_proc_is_left, graph, num_matched,
                                                   matched_edges);
 ...
 * between a opal_proc_t and a mca_btl_fftcp_proc_t instance. We cache
 * additional data (specifically the list of mca_btl_fftcp_endpoint_t instances,
 * and published addresses) associated w/ a given destination on this
 * datastructure.
 */

mca_btl_fftcp_proc_t *mca_btl_fftcp_proc_create(opal_proc_t *proc)
{
    mca_btl_fftcp_proc_t *btl_proc;
    int rc, local_proc_is_left;
    mca_btl_fftcp_modex_addr_t *remote_addrs = NULL;
    size_t size;

    OPAL_THREAD_LOCK(&mca_btl_fftcp_component.tcp_lock);
    rc = opal_proc_table_get_value(&mca_btl_fftcp_component.tcp_procs, proc->proc_name,
                                   (void **) &btl_proc);
    if (OPAL_SUCCESS == rc) {
        OPAL_THREAD_UNLOCK(&mca_btl_fftcp_component.tcp_lock);
        return btl_proc;
    ...
    btl_proc = OBJ_NEW(mca_btl_fftcp_proc_t);
    if (NULL == btl_proc) {
        rc = OPAL_ERR_OUT_OF_RESOURCE;
        goto cleanup;
    }
    ...
    OPAL_MODEX_RECV(rc, &mca_btl_fftcp_component.super.btl_version, &proc->proc_name,
    ...
    if (0 != (size % sizeof(mca_btl_fftcp_modex_addr_t))) {
        BTL_ERROR(("opal_modex_recv: invalid size %lu: btl-size: %lu\n", (unsigned long) size,
                   (unsigned long) sizeof(mca_btl_fftcp_modex_addr_t)));
        rc = OPAL_ERROR;
        goto cleanup;
    }

    btl_proc->proc_addr_count = size / sizeof(mca_btl_fftcp_modex_addr_t);
    btl_proc->proc_addrs = malloc(btl_proc->proc_addr_count * sizeof(mca_btl_fftcp_addr_t));
     ...
     */
    local_proc_is_left = mca_btl_fftcp_proc_is_proc_left(proc->proc_name,
                                                       opal_proc_local_get()->proc_name);
    rc = mca_btl_fftcp_proc_handle_modex_addresses(btl_proc, remote_addrs, local_proc_is_left);
...
cleanup:
    if (OPAL_SUCCESS == rc) {
        ...
        opal_proc_table_set_value(&mca_btl_fftcp_component.tcp_procs, proc->proc_name, btl_proc);
    } else {
        if (btl_proc) {
 ...
 * it an address.
 */
int mca_btl_fftcp_proc_insert(mca_btl_fftcp_proc_t *btl_proc, mca_btl_base_endpoint_t *btl_endpoint)
{
    mca_btl_fftcp_module_t *tcp_btl = btl_endpoint->endpoint_btl;
    mca_btl_fftcp_addr_t *remote_addr;
    int rc = OPAL_SUCCESS;
...
int mca_btl_fftcp_proc_remove(mca_btl_fftcp_proc_t *btl_proc, mca_btl_base_endpoint_t *btl_endpoint)
{
    size_t i;
    if (NULL != btl_proc) {
 ...
 * process identifier.
 */
mca_btl_fftcp_proc_t *mca_btl_fftcp_proc_lookup(const opal_process_name_t *name)
{
    mca_btl_fftcp_proc_t *proc = NULL;

    OPAL_THREAD_LOCK(&mca_btl_fftcp_component.tcp_lock);
    opal_proc_table_get_value(&mca_btl_fftcp_component.tcp_procs, *name, (void **) &proc);
    OPAL_THREAD_UNLOCK(&mca_btl_fftcp_component.tcp_lock);
    if (OPAL_UNLIKELY(NULL == proc)) {
        ...
        for (uint32_t i = 0; i < mca_btl_fftcp_component.tcp_num_btls; ++i) {
            endpoint = NULL;
            (void) mca_btl_fftcp_add_procs(&mca_btl_fftcp_component.tcp_btls[i]->super, 1, &opal_proc,
                                         &endpoint, NULL);
 ...
 * of the request.
 */
void mca_btl_fftcp_proc_accept(mca_btl_fftcp_proc_t *btl_proc, struct sockaddr *addr, int sd)
{
    OPAL_THREAD_LOCK(&btl_proc->proc_lock);
    int found_match = 0;
    ...
    for (size_t i = 0; i < btl_proc->proc_endpoint_count; i++) {
         ...
         * end we prepare for the real decision function mca_btl_fftcp_endpoint_accept. */
        ...
        (void) mca_btl_fftcp_endpoint_accept(btl_endpoint, addr, sd);
        OPAL_THREAD_UNLOCK(&btl_proc->proc_lock);
        return;
    }
    /* In this case the connection was inbound to an address exported, but was not in a CLOSED
     * state. mca_btl_fftcp_endpoint_accept() has logic to deal with the race condition that has
     * likely caused this scenario, so call it here.*/
    if (found_match) {
        (void) mca_btl_fftcp_endpoint_accept(match_btl_endpoint, addr, sd);
        OPAL_THREAD_UNLOCK(&btl_proc->proc_lock);
        return;
    }
...
/*
 * convert internal data structure (mca_btl_fftcp_addr_t) to sockaddr_storage
 *
 */
bool mca_btl_fftcp_proc_tosocks(mca_btl_fftcp_addr_t *proc_addr, struct sockaddr_storage *output)
{
...
#endif
    default:
        opal_output(0, "mca_btl_fftcp_proc: unknown af_family received: %d\n",
