 ...
 * mca_btl_fftcp_addr_t structure on remote procs. These will be used
 * for interface matching and filling out the mca_btl_base_endpoint_t
 * structure.
 */
struct mca_btl_fftcp_modex_addr_t {
...
typedef struct mca_btl_fftcp_modex_addr_t mca_btl_fftcp_modex_addr_t;

#if (__STDC_VERSION__ >= 201112L)
_Static_assert(sizeof(struct mca_btl_fftcp_modex_addr_t) == 32, "mca_btl_fftcp_modex_addr_t");
#endif
 ...
 * mca_btl_fftcp_modex_addr_t structure.
 */
struct mca_btl_fftcp_addr_t {
    union {
...
typedef struct mca_btl_fftcp_addr_t mca_btl_fftcp_addr_t;
