...
#ifndef MCA_BTL_FFTCP_ENDPOINT_H
#define MCA_BTL_FFTCP_ENDPOINT_H
...
struct mca_btl_base_endpoint_t {
    ...
    struct mca_btl_fftcp_module_t *endpoint_btl; /**< BTL instance that created this connection */
    struct mca_btl_fftcp_proc_t *endpoint_proc;  /**< proc structure corresponding to endpoint */
    struct mca_btl_fftcp_addr_t *endpoint_addr;  /**< address of endpoint */
    ...
    struct mca_btl_fftcp_frag_t *endpoint_send_frag; /**< current send frag being processed */
    struct mca_btl_fftcp_frag_t *endpoint_recv_frag; /**< current recv frag being processed */
    mca_btl_fftcp_state_t endpoint_state;            /**< current state of the connection */
...
typedef mca_btl_base_endpoint_t mca_btl_fftcp_endpoint_t;
OBJ_CLASS_DECLARATION(mca_btl_fftcp_endpoint_t);

/* Magic socket handshake string */
extern const char mca_btl_fftcp_magic_id_string[MCA_BTL_TCP_MAGIC_STRING_LENGTH];

typedef struct {
    opal_process_name_t guid;
    char magic_id[MCA_BTL_TCP_MAGIC_STRING_LENGTH];
} mca_btl_fftcp_endpoint_hs_msg_t;

void mca_btl_fftcp_set_socket_options(int sd);
void mca_btl_fftcp_endpoint_close(mca_btl_base_endpoint_t *);
int mca_btl_fftcp_endpoint_send(mca_btl_base_endpoint_t *, struct mca_btl_fftcp_frag_t *);
void mca_btl_fftcp_endpoint_accept(mca_btl_base_endpoint_t *, struct sockaddr *, int);
void mca_btl_fftcp_endpoint_shutdown(mca_btl_base_endpoint_t *);

/*
 * Diagnostics: change this to "1" to enable the function
 * mca_btl_fftcp_endpoint_dump(), below
 */
#define WANT_PEER_DUMP 0

#if OPAL_ENABLE_DEBUG && WANT_PEER_DUMP
#    define MCA_BTL_TCP_ENDPOINT_DUMP(LEVEL, ENDPOINT, INFO, MSG) \
        mca_btl_fftcp_endpoint_dump((LEVEL), __FILE__, __LINE__, __func__, (ENDPOINT), (INFO), (MSG))
